...
export function MapHomePage() {
  ...
  return (
    <div className="absolute inset-0">
      <LiveMap incidents={visible} user={coords} />
      ...
      <div className="absolute bottom-5 left-3 right-20 z-10">
        <div className="inline-flex max-w-full flex-col gap-1 rounded-card border border-civic-line bg-civic-surface/95 px-3 py-2 shadow-card">
      ...
      <Link
        to="/report"
        className="absolute bottom-5 right-3 z-10 inline-flex min-h-12 items-center rounded-card bg-civic-accent px-4 text-sm font-semibold text-white shadow-card"
      >
        + Report a hazard
      </Link>
    </div>
  ...
  return (
    <button
      ...
      className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold shadow-card ${
        active
          ? "border-civic-ink bg-civic-ink text-white"
